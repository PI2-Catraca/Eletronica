...
import cv2
import signal
from leitor_biometrico import verifica_digital, upload_template, delete_template
...
while True:
    ...
    print(presenca)
    time.sleep(1.0)
    diretorio_dats = os.listdir("./biometria")
    for arquivo in dat_list:
        if (arquivo[0] in diretorio_dats) == False:
            delete_template(finger, arquivo[1])
            dat_list.remove(arquivo)
    ...
    while(presenca <= 60.0):
        ...
        if (arquivo_dat != ultimoArquivoModificado()[0]):
            arquivo_dat = ultimoArquivoModificado()[0]
            posicao_finger = ultimoArquivoModificado()[1]
            dat_list.append(ultimoArquivoModificado())
            file = './biometria/'+ arquivo_dat
            if finger.read_templates() != adafruit_fingerprint.OK:
                for i in range(finger.templates[-1]):
                    if i != finger.templates[i]:
                        posicao_finger = i
                        break                   
            upload_template(finger, file, posicao_finger)
